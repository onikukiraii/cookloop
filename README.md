...
- **冷蔵庫管理** — 食材の在庫・残量・賞味期限を一覧管理。期限が近い食材は色で警告。定番フラグで常備食材を管理
- **献立提案（AI）** — 冷蔵庫の食材から Gemini がカテゴリ別に約10品を提案（メイン・副菜・汁物・ご飯もの）。「おまかせ」と「食材指定」の2モード。ホットクックレシピはメニュー番号と手順付き、手動調理はカテゴリー・まぜかた・設定時間も提案
- **「作った！」ボタン** — 調理完了時に使用食材の消費状態（残量変更・使い切り）を一括更新
- **買い物リスト** — 定番食材が少なくなると自動追加。献立の不足食材もワンタップでまとめて追加
- **ホットクックレシピ検索** — 公式レシピ 654 件を料理名・食材名で横断検索。お気に入り登録対応
- **調味料管理** — 調味料の残量を食材とは別に管理。残量3段階と定番フラグ
- **食材マスタ** — 食材の賞味期限目安を学習・蓄積。OpenSearch + kuromoji で表記揺れを吸収した検索
